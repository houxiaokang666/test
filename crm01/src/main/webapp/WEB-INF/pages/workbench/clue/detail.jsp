        ...
        $(function () {
            ...
            $("#activityTbody").on("click", "a", function () {
                ...
                if (window.confirm("确定删除么?")) {
                    $.ajax({
                        url: "workbench/clue/RemoveContactForClueAndActivity.do",
                        data: {
            ...
            $("#relation").click(function () {
                var checkedid = $("#ActivityId input[type=checkbox]:checked");
                var checkedActivityId = $("#ActivityId input[type=checkbox]:checked").size()
                if (checkedActivityId == 0) {
                    alert("每次至少关联一个市场活动");
                    return;
                }
                var ids = "";
                $.each(checkedid, function (index, obj) {
                    ids += "ActivityId=" + obj.value + "&";
                })
                ids = ids + "clueId=${requestScope.clue.id}"
                $.ajax({
                    url: "workbench/clue/createSaveClueActivityRelation.do",
                    data: ids,
                    ...
                    success: function (data) {
                        ...
                        if (data.code == "1") {
                            ...
                            $.each(data.data, function (index, obj) {
                                htmltext += "<tr id=\"tr_" + obj.id + "\">";
                                htmltext += "<td>" + obj.name + "</td>";
                                htmltext += "<td>" + obj.startDate + "</td>";
                                htmltext += "<td>" + obj.endDate + "</td>";
                                htmltext += "<td>" + obj.owner + "</td>";
                                htmltext += "<td><a href=\"javascript:void(0);\" activityId=\"" + obj.id + "\"  style=\"text-decoration: none;\"><span class=\"glyphicon glyphicon-remove\"></span>解除关联</a></td>";
                                htmltext += "</tr>";
                            })

                            $("#activityTbody").append(htmltext);
                        } else {
                            alert(data.message);
                        }
                    }
            ...
            $("#queryActivity").keyup(function () {
                var clueId = '${requestScope.clue.id}';
                var activitieName = $("#queryActivity").val();
                $.ajax({
                    url: "workbench/clue/relatedMarketingActivities.do",
                    data: {
                        clueId: clueId,
                    ...
                    success: function (data) {
                        ...
                        $.each(data, function (index, obj) {
                            htmltext += "<tr id='ActivityId' name=\"tr_" + obj.id + "\">";
                            htmltext += "<td><input type=\"checkbox\" VALUE=\"" + obj.id + "\"/></td>";
                            htmltext += "<td>" + obj.name + "</td>";
                            htmltext += "<td>" + obj.startDate + "</td>";
                            htmltext += "<td>" + obj.endDate + "</td>";
                            htmltext += "<td>" + obj.owner + "</td>";
    ...
    <div class="modal-dialog" role="document" style="width: 80%;">
        <div class="modal-content">
            ...
            <div class="modal-footer">
                ...
                <button type="button" id="relation" class="btn btn-primary">关联</button>
            </div>
        </div>
    </div>
</div>
...
<div style="position: relative; left: 40px; top: -30px;">
    ...
    <div style="position: relative; height: 50px; width: 500px;  top: -72px; left: 700px;">
        <button type="button" class="btn btn-default" id="toConvert"><span class="glyphicon glyphicon-retweet"></span>
            转换
        </button>
...
<div style="position: relative; top: 40px; left: 40px;">
    ...
    <c:forEach items="${requestScope.clueRemarks}" var="clueRemark">
        <div class="remarkDiv" style="height: 60px;">
            ...
            <div style="position: relative; top: -40px; left: 40px;">
                ...
                <b>${requestScope.clue.fullname}-${requestScope.clue.appellation}</b> <small
                    style="color: gray;"> ${clueRemark.editFlag=="1"?clueRemark.editTime:clueRemark.createTime}
                由${clueRemark.editFlag=="1"?clueRemark.editBy:clueRemark.createBy}${clueRemark.editFlag=="1"?"修改":"创建"}</small>
            ...
            <table class="table table-hover" style="width: 900px;">
                ...
                <c:forEach items="${requestScope.activities}" var="a">
                    <tr id="tr_${a.id}">
                        ...
                        <td>${a.endDate}</td>
                        <td>${a.owner}</td>
                        <td><a href="javascript:void(0);" activityId="${a.id}" style="text-decoration: none;"><span
